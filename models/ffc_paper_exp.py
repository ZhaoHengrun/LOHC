...
class HAM(nn.Module):
    ...
    def forward(self, x, mae_x, mask, x_seg, mae_pred_seg, patch_mask_64):
        ...
        sa_feat_embed = attn
        sa_seg_embed = attn_seg
        sa_mask_embed = attn_mask
        ...
        sa_weight_mu = attn
        sa_weight_add = attn_add

        x = x * attn + attn_add
        sa_feat = x
        return x, img_feat, mae_feat, ca_weight, ca_mae_feat, ca_fusioned_feat, sa_weight, sa_feat, sa_feat_embed, sa_seg_embed, sa_mask_embed, sa_weight_mu, sa_weight_add
...
class Hiin(nn.Module):
    ...
    def forward(self, input, mae_pred_input, patch_mask_64):
        ...
        x, img_feat, mae_feat, ca_weight, ca_mae_feat, ca_fusioned_feat, sa_weight, sa_feat, \
            sa_feat_embed, sa_seg_embed, sa_mask_embed, sa_weight_mu, sa_weight_add = self.ham(x, mae_x, mask, x_seg, mae_pred_seg, patch_mask_64)
        ...
        if padding_flag is True:
            img = img[:, :, :h, :w]
            seg = seg[:, :, :h, :w]
        return img, seg, img_feat, mae_feat, ca_weight, ca_mae_feat, ca_fusioned_feat, sa_weight, sa_feat, sa_feat_embed, sa_seg_embed, sa_mask_embed, sa_weight_mu, sa_weight_add
